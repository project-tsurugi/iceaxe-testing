 ...
 * lower function test
 */
class DbLowerTest extends DbTestTableTester {

    @BeforeAll
    static void beforeAll(TestInfo info) throws Exception {
        var LOG = LoggerFactory.getLogger(DbLowerTest.class);
    ...
    void test() throws Exception {
        var sql = "select value, lower(value) from " + TEST;

        var tm = createTransactionManagerOcc(getSession());
        tm.executeAndForEach(sql, entity -> {
            String expected = lower(entity.getStringOrNull(0));
    ...
    private String lower(String s) {
        ...
        int[] a = s.codePoints().map(c -> (0 <= c && c <= 0xff) ? Character.toLowerCase(c) : c).toArray();
